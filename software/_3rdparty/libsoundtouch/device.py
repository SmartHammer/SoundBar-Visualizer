...
class Dom:
    @staticmethod
    def get_attribute(xml_dom, attribute, default_value=None):
        if attribute in xml_dom.attributes.keys():
            return xml_dom.attributes[attribute].value
        return default_value

    @staticmethod
    def get_element_attribute(xml_dom, element, attribute, default_value=None):
        element = Dom.get_element(xml_dom, element)
    ...
    @staticmethod
    def get_elements(xml_dom, element):
        return xml_dom.getElementsByTagName(element)

    @staticmethod
    def get_element(xml_dom, element):
        elements = Dom.get_elements(xml_dom, element)
        if elements:
            return elements[0]
        return None

    @staticmethod
    def get_element_value(xml_dom, element, default_value=None):
        element = Dom.get_element(xml_dom, element)
...
class WebSocketThread(Thread):
    ...
    @atexit.register
    def _killed():
        logging.info("'CLEANLY' kill 'Websocket' worker-thread: [THREAD: %s]" % (currentThread().ident))

    def __del__(self):
        atexit.unregister(self._killed)
...
class SoundTouchDevice:
    ...
    def _on_message(self, web_socket, message):
        ...
        if dom.firstChild.nodeName == "updates":
            ...
            if action == "volumeUpdated":
                ...
                self.__run_listener(self._volume_updated_listeners, self._volume)
            if action == "nowPlayingUpdated":
                self._status = Status(action_node)
                self.__run_listener(self._status_updated_listeners, self._status)
            if action == "presetsUpdated" and action_node.hasChildNodes():
                self._presets = []
                for preset in Dom.get_elements(dom, "preset"):
                    self._presets.append(Preset(preset))
                self.__run_listener(self._presets_updated_listeners, self._presets)
            if action == "zoneUpdated":
                self.__run_listener(self._zone_status_updated_listeners, self.zone_status(True))
            if action == "infoUpdated":
                self.__init_config()
                self.__run_listener(self._device_info_updated_listeners, self._config)
    ...
    def __init_config(self):
        response = requests.get("http://" + self._host + ":" + str(self._port) + "/info")
    ...
    def start_notification(self):
        ...
        self._ws_client = websocket.WebSocketApp(
            ...
            on_error=lambda _, err: logging.error(err),
            subprotocols=['gabbo'])
    ...
    def refresh_status(self):
        ...
        response = requests.get("http://" + self._host + ":" + str(self._port) + "/now_playing")
        response.encoding = 'UTF-8'
        dom = minidom.parseString(response.text.encode('utf-8'))
    ...
    def refresh_volume(self):
        ...
        response = requests.get("http://" + self._host + ":" + str(self._port) + "/volume")
        dom = minidom.parseString(response.text)
        self._volume = Volume(dom)

    def refresh_presets(self):
        """Refresh presets."""
        response = requests.get("http://" + self._host + ":" + str(self._port) + "/presets")
        response.encoding = 'UTF-8'
        dom = minidom.parseString(response.text.encode('utf-8'))
        self._presets = []
        for preset in Dom.get_elements(dom, "preset"):
    ...
    def refresh_zone_status(self):
        """Refresh Zone Status."""
        response = requests.get("http://" + self._host + ":" + str(self._port) + "/getZone")
        dom = minidom.parseString(response.text)
        if Dom.get_elements(dom, "member"):
    ...
    def select_preset(self, preset):
        ...
        """
        requests.post('http://' + self._host + ":" + str(self._port) + '/select', preset.source_xml.encode('utf-8'))
    ...
    def select_content_item(self, source, source_account=None, location=None,
        ...
        requests.post('http://' + self._host + ":" + str(self._port) + '/select', content)
    ...
    def _create_zone(self, slaves):
        ...
        request_body = '<zone master="%s" senderIPAddress="%s">' % (self.config.device_id, self.config.device_ip)
        for slave in slaves:
            request_body += '<member ipaddress="%s">%s</member>' % (slave.config.device_ip, slave.config.device_id)
        request_body += '</zone>'
        return request_body
    ...
    def create_zone(self, slaves):
        ...
        logging.info("Creating multi-room zone with master device %s" % self.config.name)
        requests.post("http://" + self.host + ":" + str(self.port) + "/setZone", request_body)
    ...
    def add_zone_slave(self, slaves):
        ...
        logging.info("Adding slaves to multi-room zone with master device %s" % self.config.name)
        requests.post("http://" + self.host + ":" + str(self.port) + "/addZoneSlave", request_body)
    ...
    def remove_zone_slave(self, slaves):
        ...
        if self.zone_status() is None:
            raise NoExistingZoneException()
        request_body = self._get_zone_request_body(slaves)
        logging.info("Removing slaves from multi-room zone with master device %s" % self.config.name)
        requests.post("http://" + self.host + ":" + str(self.port) + "/removeZoneSlave", request_body)

    def _send_key(self, key):
        action = '/key'
        press = '<key state="press" sender="Gabbo">%s</key>' % key
        release = '<key state="release" sender="Gabbo">%s</key>' % key
        requests.post('http://' + self._host + ":" + str(self._port) + action, press)
        requests.post('http://' + self._host + ":" + str(self._port) + action, release)

    def play_media(self, source, location, source_acc=None, media_type=Type.URI):
        """
        ...
        requests.post('http://' + self._host + ":" + str(self._port) + action, play)
    ...
    def play_url(self, url):
        ...
        template_file = os.path.join(os.path.dirname(__file__), 'templates/avt_transport_uri.xml')
        with open(template_file, 'r') as template:
            body = template.read().format(url)
            requests.post("http://{0}:{1}/AVTransport/Control".format(self.host, self.dlna_port), data=body, headers=headers)
    ...
    def set_volume(self, level):
        ...
        requests.post('http://' + self._host + ":" + str(self._port) + action, volume)
...
class Config:
    ...
    def __init__(self, xml_dom):
        ...
        """
        self._id = Dom.get_element_attribute(xml_dom, "info", "deviceID")
        self._name = Dom.get_element_value(xml_dom, "name")
        self._type = Dom.get_element_value(xml_dom, "type")
        self._account_uuid = Dom.get_element_value(xml_dom, "margeAccountUUID")
        self._module_type = Dom.get_element_value(xml_dom, "moduleType")
        self._variant = Dom.get_element_value(xml_dom, "variant")
        self._variant_mode = Dom.get_element_value(xml_dom, "variantMode")
        self._country_code = Dom.get_element_value(xml_dom, "countryCode")
        self._region_code = Dom.get_element_value(xml_dom, "regionCode")
        self._networks = []
        for network in xml_dom.getElementsByTagName("networkInfo"):
            self._networks.append(Network(network))
        self._components = []
        for components in Dom.get_elements(xml_dom, "components"):
            for component in Dom.get_elements(components, "component"):
...
class Network:
    ...
    def __init__(self, network_dom):
        ...
        self._mac_address = Dom.get_element_value(network_dom, "macAddress")
        self._ip_address = Dom.get_element_value(network_dom, "ipAddress")
...
class Component:
    ...
    def __init__(self, component_dom):
        ...
        """
        self._category = Dom.get_element_value(component_dom, "componentCategory")
        self._software_version = Dom.get_element_value(component_dom, "softwareVersion")
        self._serial_number = Dom.get_element_value(component_dom, "serialNumber")
...
class Status:
    ...
    def __init__(self, xml_dom):
        ...
        """
        self._source = Dom.get_element_attribute(xml_dom, "nowPlaying", "source")
        self._content_item = None
        content_item = xml_dom.getElementsByTagName("ContentItem")
        if content_item:
            self._content_item = ContentItem(Dom.get_element(xml_dom, "ContentItem"))
        self._track = Dom.get_element_value(xml_dom, "track")
        self._artist = Dom.get_element_value(xml_dom, "artist")
        self._album = Dom.get_element_value(xml_dom, "album")
        image_status = Dom.get_element_attribute(xml_dom, "art",
                                                  "artImageStatus")
        if image_status == "IMAGE_PRESENT":
            self._image = Dom.get_element_value(xml_dom, "art")
        else:
            self._image = None

        duration = Dom.get_element_attribute(xml_dom, "time", "total")
        self._duration = int(duration) if duration is not None else None
        position = Dom.get_element_value(xml_dom, "time")
        self._position = int(position) if position is not None else None
        self._play_status = Dom.get_element_value(xml_dom, "playStatus")
        self._shuffle_setting = Dom.get_element_value(xml_dom, "shuffleSetting")
        self._repeat_setting = Dom.get_element_value(xml_dom, "repeatSetting")
        self._stream_type = Dom.get_element_value(xml_dom, "streamType")
        self._track_id = Dom.get_element_value(xml_dom, "trackID")
        self._station_name = Dom.get_element_value(xml_dom, "stationName")
        self._description = Dom.get_element_value(xml_dom, "description")
        self._station_location = Dom.get_element_value(xml_dom, "stationLocation")
...
class ContentItem:
    ...
    def __init__(self, xml_dom):
        ...
        """
        self._name = Dom.get_element_value(xml_dom, "itemName")
        self._image = Dom.get_element_value(xml_dom, "containerArt")
        self._source = Dom.get_attribute(xml_dom, "source")
        self._type = Dom.get_attribute(xml_dom, "type")
        self._location = Dom.get_attribute(xml_dom, "location")
        self._source_account = Dom.get_attribute(xml_dom, "sourceAccount")
        self._is_presetable = Dom.get_attribute(xml_dom, "isPresetable") == 'true'
    ...
    def __repr__(self):
        ...
        fields = [self.name.encode('UTF-8') if self.name else self.name,
                  self.image, self.source, self.location,
                  self.source_account, self.is_presetable]
...
class Volume:
    ...
    def __init__(self, xml_dom):
        ...
        """
        self._actual = int(Dom.get_element_value(xml_dom, "actualvolume"))
        self._target = int(Dom.get_element_value(xml_dom, "targetvolume"))
        self._muted = Dom.get_element_value(xml_dom, "muteenabled") == "true"
...
class Preset:
    ...
    def __init__(self, preset_dom):
        ...
        """
        self._name = Dom.get_element_value(preset_dom, "itemName")
        self._id = Dom.get_attribute(preset_dom, "id")
        self._source = Dom.get_element_attribute(preset_dom, "ContentItem", "source")
        self._type = Dom.get_element_attribute(preset_dom, "ContentItem", "type")
        self._location = Dom.get_element_attribute(preset_dom, "ContentItem", "location")
        self._source_account = Dom.get_element_attribute(preset_dom, "ContentItem", "sourceAccount")
        self._is_presetable = Dom.get_element_attribute(preset_dom, "ContentItem", "isPresetable") == "true"
        self._source_xml = Dom.get_element(preset_dom, "ContentItem").toxml()
...
class ZoneStatus:
    ...
    def __init__(self, zone_dom):
        ...
        """
        self._master_id = Dom.get_element_attribute(zone_dom, "zone", "master")
        self._master_ip = Dom.get_element_attribute(zone_dom, "zone", "senderIPAddress")
        self._is_master = self._master_ip is None
        members = Dom.get_elements(zone_dom, "member")
        self._slaves = []
...
class ZoneSlave:
    ...
    def __init__(self, member_dom):
        ...
        """
        self._ip = Dom.get_attribute(member_dom, "ipaddress")
        self._role = Dom.get_attribute(member_dom, "role")
